...
const AddPaymentModal = ({ order, onClose, onSuccess }: any) => {
  ...
  const handleSubmit = async () => {
    ...
    if (Number(form.amount) > Number(order?.amount_to_be_paid)) {
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-lg animate-in fade-in zoom-in duration-200">
        ...
        <div className="mb-6">
          <h2 className="text-xl font-bold text-gray-900">Add Payment</h2>
          <p className="text-sm text-gray-500 mt-1">
            Order: <span className="font-semibold text-orange-600">#{order?.id}</span>
          </p>
        </div>

        {/* 🔴 BALANCE DISPLAY */}
        <div className="mb-4 p-3 bg-red-50 border border-red-100 rounded-lg flex justify-between items-center">
          <span className="text-sm text-gray-600 font-medium">Balance Amount</span>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <input
              type="number"
              min="0"
              max={order?.amount_to_be_paid}
              placeholder="Enter amount"
              className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500"
              value={form.amount}
              onChange={(e) => {
                let val:any = Number(e.target.value);
                const max = Number(order?.amount_to_be_paid || 0);
