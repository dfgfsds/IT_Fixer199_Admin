...
const ToolsInventoryMovement: React.FC = () => {
    ...
    const fetchToolsInventoryMovements = async (
        pageNumber = page,
        size = pageSize
    ) => {
        ...
        try {
            ...
            const res = await axiosInstance.get(
                `${Api.toolsInventoryMovement}/?${params.toString()}`
            );
    ...
    useEffect(() => {
        fetchToolsInventoryMovements(page, pageSize);
    }, [startDate, endDate, statusFilter]);


    const handlePageChange = (newPage: number) => {
        setPage(newPage);
        fetchToolsInventoryMovements(newPage, pageSize);
    };

    const handlePageSizeChange = (size: number) => {
        setPageSize(size);
        setPage(1);
        fetchToolsInventoryMovements(1, size);
    };
                ...
                <div className="flex items-end">
                    <button
                        onClick={() => {
                            ...
                            fetchToolsInventoryMovements(1, pageSize);
                        }}
